...
from .__arima import load_arima_model, ArimaModel
...
class HybridPreprocessor:
    ...
    def fit(self, df: pd.DataFrame):
        ...
        self.df_cols = [self.target] + self.exoq + [col for col in df.columns if col != self.target and self.save_collumns and col not in self.exoq]
    ...
    def transform(self, df: pd.DataFrame, horison, exoq):
        ...
        exoq = torch.tensor(exoq, dtype=torch.float32).reshape(-1,1)
    ...
    def fit_transform(self, df: pd.DataFrame, horizon, exoq):
        """Обучает и применяет предобработку"""
        self.fit(df)
        return self.transform(df, horizon, exoq)
...
class HybridModel:
    def __init__(
        ...
        self.seq_len = preprocessor.seq_len


    def sarima_forecast(self, df, horizon):
        
        # start = len(df) - self.seq_len 
        delta = df.index[1] - df.index[0]
        start = df.index[-1] - delta * self.seq_len
        end = df.index[-1] + delta * horizon
        # end = len(df) + horizon
        predictions = self.sarima.get_prediction(start=start, end=end).predicted_mean
        return (predictions[:self.seq_len +1], predictions[self.seq_len +1:].values)

    def forecast(self, df: pd.DataFrame, horizon):
        """
        Делает прогноз на forecast_horizon шагов вперед.
        """
        df = df.copy()

        input_sarima, output_sarima = self.sarima_forecast(df, horizon)
        
        df["sarima"] = input_sarima
        df[f"{self.target}_diff"] = df[self.target] - df["sarima"]

        # Применяем предобработку к данным
        processed_data = self.preprocessor.fit_transform(df, horizon, exoq=output_sarima.reshape(-1,1))
        ...
        predictions = predictions.squeeze(0).cpu().detach().numpy().reshape(-1,) + output_sarima
...
def __load_hybrid_models(
    ...
    sarima = load_arima_model(f"{dir}/SARIMA.pkl", train_df,).get_model()
    for model_name in models_dict.keys():
        model_path = f"{dir}/hybrid_{model_name}.pth"
        ...
        preprocessor = HybridPreprocessor(
            target=f"{task_dict[target]}_diff",
            freq=time_delta,
...
def load_hybrid_models(
    ...
    models = []
    for param in params:
        models.extend(__load_hybrid_models(*param))
